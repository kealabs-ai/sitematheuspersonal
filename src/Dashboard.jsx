...
import {
  Dumbbell, TrendingUp, Salad, User,
  Flame, Calendar, ChevronRight, Trophy, Zap
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    dashboard.summary()
      .then(data => {
        setSummary(data);
        // Exibe onboarding se não tem flag E não tem plano ativo (week vazia)
        ...
        if (!hasFlag && !hasPlan) {
          ...
          // Já tem plano criado anteriormente, marca como concluído
  ...
  const week = (summary?.week ?? []).map(d => ({
    ...d,
    day: d.day ?? WEEK_LABEL[d.week_day] ?? '?',
    status: d.is_rest ? 'rest' : (d.status === 'pending' ? 'upcoming' : (d.status ?? 'upcoming')),
  }));
  ...
  return (
    <div className="min-h-screen sport-bg text-white font-inter pt-[60px] md:pt-[68px] pb-[60px] md:pb-6">
      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex items-center justify-between mt-1">
            <h1 className="text-4xl font-bebas text-white">
              {(user.name ?? 'Aluno').split(' ')[0]} <span className="text-lime-green">💪</span>
            </h1>
        ...
        >
          {[
            { icon: <Flame size={20} />, value: stats.streak, label: 'Dias seguidos', color: 'text-orange-400' },
            { icon: <Calendar size={20} />, value: stats.trainings_this_week, label: 'Treinos na semana', color: 'text-lime-green' },
            { icon: <Trophy size={20} />, value: stats.days_active, label: 'Dias de jornada', color: 'text-yellow-400' },
          ].map((s, i) => (
        ...
        >
          {modules.map((m, i) => {
            ...
            return locked ? (
              <div
                key={i}
                className="bg-dark-card border-2 border-dark-border p-5 text-left opacity-50 relative overflow-hidden cursor-not-allowed"
              >
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs text-lime-green font-bold">{badgesEarned}/{badgesTotal}</span>
          </div>

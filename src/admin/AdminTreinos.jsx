...
export default function AdminTreinos() {
  ...
  const pid = (p) => p.id ?? p.id_user;
  const did = (d) => d.id ?? d.id_user;
  const eid = (e) => e.id ?? e.id_user;
  ...
  const saveDay = async () => {
    ...
    if (!day) {
      ...
      if (updated) setDays(prev => ({ ...prev, [planId]: Array.isArray(updated) ? updated : (updated?.days ?? updated?.data ?? []) }));
    } else {
  ...
  const saveEx = async () => {
    ...
    if (!ex) {
      ...
      if (updated) setExercises(prev => ({ ...prev, [dayId]: Array.isArray(updated) ? updated : (updated?.exercises ?? updated?.data ?? []) }));
    } else {
          ...
          {plans.map((plan, pi) => (
            <div key={pid(plan) ?? pi} className="border border-dark-border bg-dark-card overflow-hidden">
              ...
              <div className="flex items-center justify-between px-4 py-3">
                <button onClick={() => togglePlan(pid(plan))} className="flex items-center gap-3 flex-1 text-left">
                  {expanded === pid(plan) ? <ChevronDown size={16} className="text-lime-green" /> : <ChevronRight size={16} className="text-gray-500" />}
                  <div>
              ...
              {expanded === pid(plan) && (
                <div className="border-t border-dark-border bg-black/30 px-4 py-3 space-y-2">
                  ...
                  {(days[pid(plan)] ?? []).map((day, di) => (
                    <div key={did(day) ?? di} className="border border-dark-border bg-dark-card">
                      <div className="flex items-center justify-between px-3 py-2.5">
                        <button onClick={() => loadExercises(did(day))}
                        ...
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => { setDayForm({ name: day.name, day_of_week: day.day_of_week ?? day.week_day ?? 'SEG', duration_min: day.duration_min, is_rest: day.is_rest ?? false }); setDayModal({ planId: pid(plan), day }); }}
                        ...
                        <div className="border-t border-dark-border px-3 pb-2 pt-1 space-y-1">
                          {exercises[did(day)].map((ex, ei) => (
                            <div key={eid(ex) ?? ei} className="flex items-center justify-between py-1.5 border-b border-dark-border/50 last:border-0">
                              ...
                              <div className="flex gap-1">
                                <button onClick={() => { setExForm({ name: ex.name, sets: ex.sets, reps: ex.reps, rest_seconds: ex.rest_seconds, muscle_group: ex.muscle_group, video_url: ex.video_url ?? '', notes: ex.notes ?? '' }); setExModal({ dayId: did(day), ex }); }}
